...
export function rowToPage(row: Record<string, unknown>): Page {
  ...
  return {
    ...
    lifecycle_status: row.lifecycle_status as Page['lifecycle_status'] | undefined,
    ...
    valid_from: readOptionalDate(row.valid_from),
    valid_until: readOptionalDate(row.valid_until),
    authored_at: readOptionalDate(row.authored_at),
    superseded_by_slug: row.superseded_by_slug == null ? null : String(row.superseded_by_slug),
